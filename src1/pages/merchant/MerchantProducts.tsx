...
export default function MerchantProducts() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      // Client-side filter to avoid JSON Server type mismatch (string vs number)
      const all: any[] = res.data;
      const mine = all.filter((p) => String(p.seller_id) === String(user?.id));
      setProducts(mine as Product[]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggleActive = async (product: Product) => {
    try {
      ...
      setProducts((p) => p.map((x) => String(x.id) === String(product.id) ? { ...x, is_active: !x.is_active } : x));
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 bg-gray-100 dark:bg-[#252525] rounded-xl overflow-hidden shrink-0 flex items-center justify-center">
                          {p.main_image
                            ...
                            : <ShoppingBag className="w-5 h-5 text-gray-300 dark:text-gray-600" />
                          }
                        </div>
                        <div>
