    ...
    if (isEdit && id) {
      ...
      axios.get(`${DB}/products/${id}`).then((r) => {
        ...
        setForm({
          ...
          brand: PRESET_BRANDS.includes(p.brand ?? '') ? (p.brand ?? '') : p.brand ? 'other' : '',
          brand_other: PRESET_BRANDS.includes(p.brand ?? '') ? '' : (p.brand ?? ''),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const productData = {
        ...
        main_image: variants.find((v) => v.is_main)?.image || variants[0]?.image || null,
        ...
        variants: variants.map((v, i) => ({
          ...
          discount: v.old_price && v.price
            ? Math.round((1 - Number(v.price) / Number(v.old_price)) * 100)
            : 0,
          stock: Number(v.stock),
          image: v.image || null,
          is_main: v.is_main,
          is_active: v.is_active,
          is_in_stock: Number(v.stock) > 0,
          attributes: v.attributes,
        })),
