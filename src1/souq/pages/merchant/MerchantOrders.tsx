...
import type { Order, OrderStatus } from '@souq/types';
...
export default function MerchantOrders() {
  ...
  useEffect(() => {
    if (!user?.id) return;
    axios.get(`${DB}/orders`).then((r) => {
      const allOrders: any[] = r.data;
      // فلتر: فقط الطلبيات التي تحتوي على منتج يخص هذا التاجر
      const myOrders = allOrders.filter((order) =>
        (order.items ?? []).some((item: any) => String(item.seller_id) === String(user.id))
      );
      setOrders(myOrders);
