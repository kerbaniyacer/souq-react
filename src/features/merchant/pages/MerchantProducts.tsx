...
import { Plus, Pencil, Trash2, Eye, EyeOff, ShoppingBag, Search, ShieldAlert, Clock, RotateCcw, AlertCircle } from 'lucide-react';
...
export default function MerchantProducts() {
  ...
  const handleResubmit = async (id: string | number) => {
    if (!confirm('هل تريد إعادة تقديم هذا المنتج للمراجعة؟')) return;
    setResubmitting(String(id));
    try {
      await productsApi.resubmit(Number(id));
      toast.success('تم إعادة تقديم المنتج للمراجعة');
      fetchProducts();
    } catch {
      toast.error('تعذر إعادة تقديم المنتج');
    } finally {
                  ...
                  <tr key={p.id} className="hover:bg-gray-50/50 dark:hover:bg-[#252525]/50 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      {p.status === 'under_review' ? (
                        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-arabic font-bold bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400">
                        ...
                        <div className="flex flex-col gap-1">
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] font-arabic font-bold bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400">
                            <AlertCircle className="w-3 h-3" /> مرفوض — يحتاج تعديل
                          </span>
                          {(p as any).suspension_reason && (
                            <p className="text-[10px] text-gray-400 dark:text-gray-500 font-arabic px-1 max-w-[140px] truncate" title={(p as any).suspension_reason}>
                              {(p as any).suspension_reason}
                            </p>
                          )}
                        </div>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        {/* Resubmit — only for rejected products */}
                        {p.status === 'rejected' && (
                          <button
                            onClick={() => handleResubmit(p.id)}
                            disabled={resubmitting === String(p.id)}
                            className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-arabic font-bold text-white bg-primary-500 hover:bg-primary-600 disabled:opacity-60 rounded-lg transition-colors"
                            title="إعادة تقديم للمراجعة بعد التعديل"
                          >
                            {resubmitting === String(p.id)
                              ? <span className="w-3.5 h-3.5 border-2 border-white/40 border-t-white rounded-full animate-spin block" />
                              : <RotateCcw className="w-3.5 h-3.5" />
                            }
                            <span className="hidden lg:inline">إعادة تقديم</span>
                          </button>
                        )}

                        {/* Preview — hide for under_review / rejected */}
                        {p.status !== 'under_review' && p.status !== 'rejected' && (
                          <Link
                        ...
                        {/* Edit — available always except under_review */}
                        {p.status !== 'under_review' && (
