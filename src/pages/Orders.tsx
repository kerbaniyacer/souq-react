  ...
  if (loading) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-10 space-y-4">
        {[...Array(3)].map((_, i) => <div key={i} className="h-28 bg-gray-100 dark:bg-[#252525] rounded-2xl animate-pulse" />)}
      </div>
    );
  }
        ...
        {orders.map((order) => {
          ...
          return (
            <Link
              key={order.id}
              to={`/orders/${order.id}`}
              className="block bg-white dark:bg-[#1E1E1E] rounded-2xl border border-gray-100 dark:border-[#2E2E2E] p-5 hover:border-primary-200 dark:hover:border-primary-800 hover:shadow-md transition-all"
            >
              ...
              {order.items?.length > 0 && (
                <div className="mt-3 pt-3 border-t border-gray-100 dark:border-[#2E2E2E]">
