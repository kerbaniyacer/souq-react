...
import { loginWithGoogle, loginWithFacebook } from '@/services/mockAuth';
...
const isGoogleConfigured  = !!(GOOGLE_CLIENT_ID  && !GOOGLE_CLIENT_ID.startsWith('your-')  && GOOGLE_CLIENT_ID.includes('.apps.googleusercontent.com'));
const isFacebookConfigured = !!(FACEBOOK_APP_ID && !FACEBOOK_APP_ID.startsWith('your-'));

// ─── المكوّن الداخلي: يحتوي على hooks — يُعرض فقط عندما يكون OAuth مُهيَّأ ────
function OAuthButtons({ mode }: Props) {
  ...
  const handleFacebookClick = async () => {
    if (!isSdkLoaded) {
      toast.error('Facebook SDK لا يزال يُحمَّل، انتظر لحظة وحاول مجدداً');
      return;
    }
    setFbLoading(true);
    try {
  ...
  return (
    <>
      {isGoogleConfigured && (
        <button
          type="button"
          onClick={() => { setGoogleLoading(true); loginGoogle(); }}
...
// ─── المكوّن الخارجي: حارس — يُظهر أزرار OAuth فقط إذا كانت مُهيَّأة ─────────
export default function SocialAuthButtons({ mode }: Props) {
  const label = mode === 'login' ? 'دخول' : 'تسجيل';

  // لا يوجد أي OAuth مُهيَّأ → لا نعرض أي شيء
