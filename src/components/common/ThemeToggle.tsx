  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden rounded-full pointer-events-none">
        {[
          { top: '8px', left: '12px', size: 'w-1 h-1', delay: 500 },
          { top: '16px', left: '8px', size: 'w-0.5 h-0.5', delay: 700 },
          { top: '10px', left: '42px', size: 'w-0.5 h-0.5', delay: 600 },
          { top: '22px', left: '14px', size: 'w-1 h-1', delay: 800 },
        ].map((star, i) => (
          <span
            key={i}
            className={`absolute ${star.size} bg-white rounded-full transition-opacity`}
            style={{
              top: star.top,
              left: star.left,
              opacity: isDark ? [1, 0.7, 0.5, 0.6][i] : 0,
              transitionDuration: `${star.delay}ms`,
            }}
          />
        ))}
      </div>
      ...
      >
        {isDark ? (
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" style={{ filter: 'drop-shadow(0 1px 2px rgba(0,0,0,0.3))' }}>
            <path d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z" fill="#6366f1" stroke="#4f46e5" strokeWidth="1" />
            <path d="M21 12.79A9 9 0 1111.21 3 7 7 0 0021 12.79z" fill="url(#moonG)" />
            <defs>
              <linearGradient id="moonG" x1="0%" y1="0%" x2="100%" y2="100%">
          ...
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" style={{ filter: 'drop-shadow(0 1px 2px rgba(0,0,0,0.2))' }}>
            ...
            <circle cx="12" cy="12" r="5" fill="url(#sunG)" />
      ...
      {!isDark && (
        <>
          <div className="absolute w-3 h-1.5 rounded-full bg-white/40" style={{ top: '12px', right: '10px' }} />
          <div className="absolute w-2 h-1 rounded-full bg-white/30" style={{ top: '20px', right: '18px' }} />
        </>
      )}
    </button>
  );
